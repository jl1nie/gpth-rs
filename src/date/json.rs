...
pub fn build_json_date_map(
    ...
    for (json_path, json_bytes) in json_entries {
        if let Some(date) = parse_google_json(json_bytes) {
            // The JSON filename tells us which media file it belongs to
            // e.g. "Takeout/Google Photos/Photos from 2023/IMG_1234.jpg.json"
            // -> media file is "IMG_1234.jpg" in the same directory
...
fn bracket_swap(filename: &str) -> String {
    if let Some(m) = BRACKET_RE.find_iter(filename).last() {
        ...
        // Remove last occurrence of bracket (without dot)
